...
def auth():
    ...
    if choice == 2:
        ...
        api_token = click.prompt('Enter your Personal API Token', hide_input=True, type=str)
        ...
        client_secret = click.prompt('Enter your ClickUp Client Secret', hide_input=True, type=str)
...
def config(team_id, default_list, api_token, show):
    ...
    if show:
        ...
        click.echo(f"  Default List ID: {config_manager.get('user.default_list_id', 'Not set')}")
        click.echo(f"  User ID: {config_manager.get('user.user_id', 'Not set')}")
        click.echo(f"  Authenticated: {'Yes' if config_manager.is_authenticated() else 'No'}")
